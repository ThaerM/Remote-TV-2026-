...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Android TV / Google TV | mDNS (`_androidtvremote2._tcp`) | Cert-based pairing, 6-hex-digit code shown on TV | TCP + protobuf (Android TV Remote protocol v2) | Yes (D-pad, media, volume when device reports it) | Yes | No (not implemented - see docs/research/android-google-tv.md) | App-link deep-link only, not full launcher control | Via Cast (separate protocol) | No general API | Not implemented (no WoL support in this provider) | Yes | Not implemented | Unofficial but stable, widely implemented (Home Assistant, python `androidtvremote2`) | No official Flutter/Dart SDK; protobuf bindings regenerated from androidtvremote2's compiled descriptors, own TLS/pairing implementation | Medium | High - **implemented in Phase 1, not yet physical-device validated** |
